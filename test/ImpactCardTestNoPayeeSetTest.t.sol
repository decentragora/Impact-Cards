...
contract ImpactCardTest is Test {
    ...
    function testPayAllWontSendToZeroAddrs() public {
        vm.startPrank(dAgoraTreasury);
            uint256[2] memory shares = [uint256(50), uint256(50)];
            cards.setPayees(1, [dAgoraTreasury, ag], shares);
        vm.stopPrank();
        vm.startPrank(alice);
        ...
        vm.startPrank(dAgoraTreasury);
            ...
            // assert tokens that have payees not set are not sent to zero address
            assertEq(cards.getAccumulatedFunds(1)[0], 0);
            ...
            assertEq(cards.getAccumulatedFunds(2)[1], idBalances[1]);
            assertEq(cards.getAccumulatedFunds(5)[1], idBalances[2]);
            assertEq(cards.getAccumulatedFunds(10)[1], idBalances[3]);
